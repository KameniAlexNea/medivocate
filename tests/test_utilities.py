...
class TestLoadData:
    ...
    def test_download_and_prepare_data_success(
        self,
        mock_subprocess,
        mock_zipfile,
        mock_remove,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            download_and_prepare_data(
                gdrive_url, zip_filename, extract_to, target_folder
            )

            # Verify subprocess.run was called for gdown
            mock_subprocess.assert_called_once_with(
                ["gdown", gdrive_url, "-O", zip_filename], check=True
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("src.utilities.load_data.logger") as mock_logger:
                ...
                mock_logger.info.assert_called_with(
                    f"Data already exists in {target_folder}"
                )
    ...
    def test_download_failure(self, mock_exists, mock_subprocess):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("src.utilities.load_data.logger") as mock_logger:
                download_and_prepare_data("test_url", "test.zip", temp_dir, temp_dir)
