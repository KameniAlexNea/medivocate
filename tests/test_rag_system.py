...
class TestRAGSystem:
    ...
    def test_load_documents_failure(self, mock_vs_manager, mock_get_llm, mock_get_emb):
        ...
        mock_vs_instance = MagicMock()
        mock_vs_manager.return_value = mock_vs_instance
        mock_vs_instance.load_and_process_documents.side_effect = Exception("Load failed")
    ...
    @patch('src.rag_pipeline.rag_system.get_llm_model_chat')
    @patch('src.rag_pipeline.rag_system.VectorStoreManager')
    def test_initialize_vector_store_success(self, mock_vs_manager, mock_get_llm, mock_get_emb):
    ...
    @patch('src.rag_pipeline.rag_system.get_llm_model_chat')
    @patch('src.rag_pipeline.rag_system.VectorStoreManager')
    def test_initialize_vector_store_failure(self, mock_vs_manager, mock_get_llm, mock_get_emb):
        """Test vector store initialization failure."""
        mock_llm = MagicMock()
        mock_get_llm.return_value = mock_llm
        mock_vs_instance = MagicMock()
        mock_vs_manager.return_value = mock_vs_instance
        mock_vs_instance.initialize_vector_store.side_effect = Exception("VS init failed")
