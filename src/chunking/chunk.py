...
class ChunkingManager:
    ...
    def is_valid_file(self, raw: str):
        ...
        count3 = [1 for i in lines if self.is_potential_title(i)]  # potential citation part
        return (
            (50 > len(lines) > 10)
            and len(count) < 5
            and len(count2) < 5
    ...
    def generate_keywords(self, paragraphs: str, use_llm=True):
        ...
        for paragraph in paragraphs:
            ...
            if use_llm:
                ...
                keywords = kw_model.extract_keywords(paragraph, keyphrase_ngram_range=(1, 2), stop_words="french", top_n=self.nb_keywords)
    ...
    def retrieve_documents_from_file(self, file_path: str, use_llm_for_keywords=True, summarize_before_chunk=True, check_text_validity=True, verbose=False):
...
if __name__ == "__main__":
    ...
    chunkingManager.retrieve_documents_from_file(file_path=file_path, verbose=False, use_llm_for_keywords=True, summarize_before_chunk=True, check_text_validity=False)
    
    
    for use_llm in [True, False]:
        for summ_before_chunk in [True, False]:
            print("***************************************************************************************************")
            print("use_llm_for_keyword: {}; summarize_before_chunk: {}".format(use_llm, summ_before_chunk))
            print("***************************************************************************************************")
            start = time.time()
            documents = chunkingManager.retrieve_documents_from_file(file_path=file_path, verbose=True, use_llm_for_keywords=use_llm, summarize_before_chunk=summ_before_chunk, check_text_validity=False)
            durations.append(["use_llm_for_keyword: {}; summarize_before_chunk: {}".format(use_llm, summ_before_chunk), round(time.time()-start, 5)])
            print("\n")
